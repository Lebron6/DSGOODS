 ...
 * 普通箱已绑货
 */
public class BoxAlreadyBindGoodsActivity extends BaseActivity {
    ...
    public static void actionStart(Context context, String num, String pkid) {
        Intent intent = new Intent(context, BoxAlreadyBindGoodsActivity.class);
    ...
    protected int attachLayoutRes() {
        return R.layout.activity_box_already_bind_goods;
    }
    ...
    private void getData() {
        HttpUtil.createRequest(TAG, BaseUrl.getInstence().enchasePackingGet()).enchasePackingGet(PreferenceUtils.getInstance().getUserToken(), getIntent().getStringExtra(NUM), getIntent().getStringExtra(PKID)).enqueue(new Callback<ApiResponse<TurnoverInfo>>() {
            ...
            public void onFailure(Call<ApiResponse<TurnoverInfo>> call, Throwable t) {
                ToastUtil.showToast("网络异常:获取普通箱绑货信息失败");
            }
        });
    }
    ...
    private void unBind() {
        HttpUtil.createRequest(BaseUrl.getInstence().packingUnbind()).packingUnbind(PreferenceUtils.getInstance().getUserToken(), getIntent().getStringExtra(NUM),getIntent().getStringExtra(PKID)
    ...
    private void updata() {
        HttpUtil.createRequest(BaseUrl.getInstence().packingEditSave()).packageEditSave(PreferenceUtils.getInstance().getUserToken(), getIntent().getStringExtra(NUM),
                getIntent().getStringExtra(PKID), etHwsl.getText().toString()).enqueue(new Callback<ApiResponse>() {
            @Override
            public void onResponse(Call<ApiResponse> call, Response<ApiResponse> response) {
                if (response.body().getCode() == 1) {
