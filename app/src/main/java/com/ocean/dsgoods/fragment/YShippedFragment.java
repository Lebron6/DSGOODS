 ...
 * 我发运的-已完成
 */
public class YShippedFragment extends BaseFragment {
    ...
    protected void initViews() {
        initSpringViewStyle();
          adapter = new ShippedAdapter(getActivity(),2);
    }
    ...
    private void getData() {
        HttpUtil.createRequest(TAG, BaseUrl.getInstence().waybillList()).waybillList(PreferenceUtils.getInstance().getUserToken(), page + "", "2", 15 + "").enqueue(new Callback<ApiResponse<WayList>>() {
            @Override
